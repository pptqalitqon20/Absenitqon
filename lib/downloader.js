...
async function handleTiktokVideo(sock, chat, url, quotedMsg) { // <-- TAMBAH quotedMsg
  ...
  try {
    ...
    await sendText(sock, chat, "*Dari pada buka linknya, mending langsung nonton nih videonya, sudah saya ubah ke video siap nonton 👆🏻*."); // <-- Teruskan quotedMsg
    return true;
  } catch (e) {
...
async function handleTiktokAudio(sock, chat, url, quotedMsg) { // <-- TAMBAH quotedMsg
  ...
  try {
    ...
    await sendText(sock, chat, "✅ Alhamdulillah Sudah Selesai Barakallahu Fiikum, Berikut Hasilnya 👇."); // <-- Teruskan quotedMsg
    return true;
  } catch (e) {
