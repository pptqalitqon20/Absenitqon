...
async function handleYtVideo(sock, chat, url, quotedMsg) {
  ...
  await sendText(sock, chat, "❌ Gagal mendownload video dari YouTube.", quotedMsg);
  return true;
}
...
async function handleInstagram(sock, chat, url, quotedMsg) { // <-- TAMBAH quotedMsg
  ...
  try {
    ...
    if (!hasil || !hasil.length) {
      await sendText(sock, chat, "❌ Postingan tidak tersedia atau akun privat.", quotedMsg); // <-- Teruskan quotedMsg
      return true;
    }
    ...
    await sendText(sock, chat, "❌ Gagal mengambil postingan Instagram.", quotedMsg); // <-- Teruskan quotedMsg
    return true;
  }
}
...
async function handleInstaStory(sock, chat, username, quotedMsg) { // <-- TAMBAH quotedMsg
  ...
  try {
    ...
    if (!hasil || !hasil.results || !hasil.results.length) {
      await sendText(sock, chat, "❌ Story tidak ditemukan atau akun privat.", quotedMsg); // <-- Teruskan quotedMsg
      return true;
    }
    ...
    await sendText(sock, chat, "❌ Username tidak ditemukan atau privat.", quotedMsg); // <-- Teruskan quotedMsg
    return true;
  }
}
...
async function handleTiktokVideo(sock, chat, url, quotedMsg) { // <-- TAMBAH quotedMsg
  ...
  try {
    ...
    await sendText(sock, chat, "❌ Gagal / URL TikTok tidak valid atau media tidak bisa diambil.", quotedMsg); // <-- Teruskan quotedMsg
    return true;
  }
}
...
async function handleTiktokAudio(sock, chat, url, quotedMsg) { // <-- TAMBAH quotedMsg
  ...
  try {
    ...
    await sendText(sock, chat, "❌ Gagal / URL TikTok tidak valid atau audio tidak bisa diambil.", quotedMsg); // <-- Teruskan quotedMsg
    return true;
  }
}
...
async function handleFacebookVideo(sock, chat, url, quotedMsg) { // <-- Menerima quotedMsg
  ...
  await sendText(sock, chat, "⏳ Sedang memproses video dari Facebook...", quotedMsg);
  
  try {
    const hasil = await facebookDl(url);
    if (!hasil || !Array.isArray(hasil.results) || hasil.results.length === 0) {
      await sendText(sock, chat, "❌ Video tidak ditemukan atau tidak bisa diambil.", quotedMsg);
      return true;
    }
    ...
    await sendText(sock, chat, "❌ Server downloader Facebook sedang offline / error.", quotedMsg);
    return true;
  }
}
