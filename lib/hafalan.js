...
async function handleHafalanSelection(sock, m, kode) {
  ...
  try {
    ...
    const namaUstadz = sheetData[barisAwal][1]?.trim() || "Belum terisi";
    ...
    for (let i = barisSantriStart; i < barisAwal + 15; i++) {
      ...
      const namaSantri = row[0].trim();
      const hafalan = row[1]?.trim() || "0";
      const juz = row[2]?.trim() || "-";
    ...
    let pesan =
      ...
      `│👥 *Halaqah* : ${namaHalaqah}\n` +
      `│👳🏻‍♂️ *Ustadz* : ${namaUstadz}\n` +
      `│📌 *Jumlah Santri* : ${jumlahSantri} Orang\n` +
      `│🗓️ *Hari/Tanggal* : ${tanggalHariIni}\n` +
      `╰────\n\n` +
      `╭──「 👥 *DAFTAR SANTRI* 」\n` +
      daftarSantri +
      `╰────\n\n` +
    ...
    // optional: hapus session setelah tampil
