...
function getTanggalHariIni() {
  ...
  const now = new Date();
  const hari = hariDict[now.getDay()];
  const tanggal = now.getDate();
  const bulan = bulanDict[now.getMonth()];
...
//    → kirim teks + tombol LIST halaqah
// ======================================
async function startHafalanFlow(sock, jid) {
  ...
  try {
    ...
    // Simpan session: nanti dipakai saat user pilih salah satu halaqah
