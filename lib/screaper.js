  ...
  download: async (link, format) => {
    if (!link) {
      return {
        status: false,
        code: 400,
        error: "Infokan Link Youtubenya Dulu😊"
      };
    }
    ...
    if (!format || !allFormats.includes(format)) {
      return {
        status: false,
        code: 400,
        error: "Itu bukan formats yang benar, liat dibawah ini👇",
    ...
    if (!id) {
      return {
        status: false,
        code: 400,
        error: "Afwan link youtubenya ada yang salah.."
      };
    }
    ...
    try {
      ...
      return {
        ...
        code: 200,
        result: {
          title: decrypted.title || "Afwan Tidak Tahu 🤷🏻",
