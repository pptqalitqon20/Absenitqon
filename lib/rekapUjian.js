...
/* ===============================
   HANDLE COMMAND
================================= */
async function handleRekapUjianCommand(sock, jid, text = '') {
  let pdfPath = null;
  ...
  try {
    ...
    const args = parts.slice(1); // setelah angka 5
    ...
    // Cari pola YYYY-MM
    const ymArg = args.find((a) => /^\d{4}-\d{2}$/.test(a));

    if (ymArg) {
      const [y, m] = ymArg.split('-');
      year = parseInt(y);
      month = parseInt(m);

      // hapus arg bulan dari list
      const remaining = args.filter((a) => a !== ymArg);
      if (remaining.length) {
        halaqah = remaining.join(' ');
      }
    } else if (args.length) {
      // Kalau tidak ada YYYY-MM tapi ada teks → anggap itu halaqah
      halaqah = args.join(' ');
    }
    ...
    if (year && month) {
      rows = await sheetsService.listAbsensiByMonth({
        year,
        month,
        halaqah,
      });
    } else {
      rows = await sheetsService.listAllAbsensi();

      if (halaqah) {
        const h = halaqah.toLowerCase();
        rows = rows.filter((r) =>
          r.halaqah.toLowerCase().includes(h)
        );
      }
    }
    ...
    if (!rows || rows.length === 0) {
      await sock.sendMessage(jid, {
        text: 'Tidak ditemukan data sesuai filter yang diminta.',
      });
      return true;
    }
    ...
    const loadingMsg = await sock.sendMessage(jid, {
      text: '⏳ Sedang menyiapkan rekap... Mohon tunggu 😊',
    });
